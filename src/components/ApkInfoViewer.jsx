...
const XmlTree = ({ manifest }) => {
  ...
  return (
    <div style={{ ...xmlFont, overflow: 'auto' }}>
      ...
      <XmlTreeNode tag='manifest' data={rootData} defaultExpanded />
    </div>
  )
}
...
const ApkInfoViewer = ({ apkInfo, fileInfo, apkFiles, onReset }) => {
  ...
  const versionCode = basicInfo.versionCode
  const minSdk = basicInfo.minSdk || 'N/A'
  const targetSdk = basicInfo.targetSdk || 'N/A'
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {activeTab === 'files' && apkFiles && (
          <ApkFileList
            files={apkFiles.files}
            nativeLibs={apkFiles.nativeLibs}
            supportedABIs={apkFiles.supportedABIs}
          />
        )}

        {activeTab === 'files' && !apkFiles && (
          <div style={{ padding: '40px', textAlign: 'center', color: '#999' }}>无文件数据</div>
        )}
